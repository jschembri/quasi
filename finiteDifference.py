...
data= subprocess.Popen('./finiteDifference %s' % calculated_time, shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE,stdin=subprocess.PIPE).communicate()
data = data[0].split(",")
...
plt.subplot(412)
plt.plot(x_values, y_values, mfc='red', ms=12, label='Density',linewidth=4)
plt.ylabel('Density')

plt.subplot(413)
plt.plot(x_values, y_velocity_values, mfc='red', ms=12, label='Velocity',linewidth=4)
plt.ylabel('Velocity')
