...
//	cout << "c1: " << pow(fluid_gamma*(fluid_gamma-1.0)*e0,0.5) << endl;
//	cout << "c2: " << pow(fluid_gamma*P0/row0,0.5) << endl;
...
// loop and work
  ...
  // Q Vector Initialization
   ...
   for (int i=0;i<=x_spaces;i++){
         Q[i][0] = 0;
			if (i==0){
		   	Q[i][1] = P0*(area(x_value[i] + delta_x) - area(x_value[i])); 
			}else if (i==x_spaces){
		   	Q[i][1] = P0*(area(x_value[i]) - area(x_value[i]-delta_x)); 
			}else{
	...
	// Finite Volume analysis
		...
		for(int i=1;i<=x_spaces;i++){
			...
			Q[i][0] = 0;
			if (i==x_spaces){
				Q[i][1] = pressure[i]*(area(x_value[i]) - area(x_value[i] - delta_x)); 
			}else{
