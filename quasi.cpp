...
int main(int argc, char **argv){
   ...
   double e0,einf;
   e0 = P0/(row0*(fluid_gamma-1)); 
   einf = Pinf/(rowinf*(fluid_gamma-1));     
...
// Resetting all variables for next cycle
	...
	for(int i=0;i<=x_spaces;i++){
   	myfile <<i<<"|"<<row[i]<<"|"<<velocity[i]<<"|"<<energy[i]<<"|"<<pressure[i]<<"|"<<F[i][0]<<"|"<<F[i][1]<<"|"<<F[i][2]<<"|"<<area(x_value[i]+delta_x/2.0)<<"|"<<area(x_value[i]-delta_x/2.0)<<"|"<<volume[i]<<endl; 
	}
   time += delta_t;
}
...
// loop and work
 ...
 printarray (volume,x_spaces+1, "Area");
