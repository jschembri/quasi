...
// ---------------------------------
   double M0,c;
	...
	c = pow(fluid_gamma*Pend/row0,0.5); 
	M0 = ustar /c;
	...
	// Finite Volume analysis
		...
		for (int i=0; i<=x_spaces+3; i++){
			if (i==x_spaces+2){
				...
				row[i]= Uplus1[i+1][0];
				velocity[i] = Uplus1[i+1][1]/row[i];
				energy[i] = Uplus1[i+1][2];
				epsilon[i] = energy[i+1]/row[i+1] - pow(velocity[i+1],2)/2.0;
				pressure[i+1] = (fluid_gamma -1.0)*row[i+1]*epsilon[i+1];
			}else{
				row[i]= Uplus1[i][0];
